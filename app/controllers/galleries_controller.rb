    ...
    respond_to do |format|
      ...
      format.xml do
        ...
        @galleries.each do |gallery|
          ...
          hash[:thumbnail] = gallery.gallery_items[0].image.url(:iphone_thumb).gsub(/\?.*$/, "").gsub(" ", "%20")
          hash[:url] = "/galleries/#{gallery.id}.xml"
          hash[:id] = gallery.id
          result.push hash
        end
        render :xml => result
    ...
    respond_to do |format|
      ...
      format.xml do
        ...
        @gallery.gallery_items.each do |item|
          hash = Hash.new
          hash[:image_url] = item.image.url(:bb).gsub(/\?.*$/, "").gsub(" ", "%20")
          hash[:thumbnail_url] = item.image.url(:iphone_thumb).gsub(/\?.*$/, "").gsub(" ", "%20")
